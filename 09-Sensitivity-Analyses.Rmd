...
IdentifyOutlier<-function(x){
  ...
  ifelse(x$HatQualCro>=3*mean(x$HatQualAcu), 1, 0)->x$HatQualAcuOUT
  ifelse(x$HatQualCro>=3*mean(x$HatQuantCro), 1, 0)->x$HatQuantCroOUT
  ifelse(x$HatQualCro>=3*mean(x$HatQuantAcu), 1, 0)->x$HatQuantAcuOUT
  
  #Identify large cooks' distance (cut off = 4/sample size)
  ifelse(x$CookQualCro>=4/nrow(x), 1, 0)->x$CookQualCroOUT
  ifelse(x$CookQualCro>=4/nrow(x), 1, 0)->x$CookQualAcuOUT
  ifelse(x$CookQualCro>=4/nrow(x), 1, 0)->x$CookQuantCroOUT
  ifelse(x$CookQualCro>=4/nrow(x), 1, 0)->x$CookQuantAcuOUT
  x}
```
...
ID.Cook<-subset(Data_Meta_S4_transit2$ResponseId,
...
Data_Meta_Cook<-filter(Data_Meta_Raw.wide, !(ResponseId %in% ID.Cook)) 
...
ID.Hat<-subset(Data_Meta_S4_transit2$ResponseId,
...
# Exclude influential participants
Data_Meta_Hat<-filter(Data_Meta_Raw.wide, !(ResponseId %in% ID.Hat)) 
